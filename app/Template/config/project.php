...
<form method="post" action="<?= $this->url->href('config', 'project') ?>" autocomplete="off">
    ...
    <p class="form-help"><?= t('Alternate values: "Backlog, Ready, WIP, Done"') ?></p>
    ...
    <p class="form-help"><?= t('Example: "Research, Write, Edit, Design, Update, Improve, Collaborate"') ?></p>

    <?php //<?= $this->form->checkbox('subtask_restriction', t('Allow only one subtask in progress at the same time for a user'), 1, $values['subtask_restriction'] == 1) ?>
    <?php //<?= $this->form->checkbox('subtask_time_tracking', t('Trigger automatically subtask time tracking'), 1, $values['subtask_time_tracking'] == 1) ?>
